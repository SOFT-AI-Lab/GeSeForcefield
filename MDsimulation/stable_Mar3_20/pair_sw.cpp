...
void PairSW::read_file(char *file)
{
  ...
  while (1) {
    ...
    params[nparams].beta = atof(words[9]);
    params[nparams].biga = atof(words[10]);
    params[nparams].bigb = atof(words[11]);
    params[nparams].powerp = atof(words[12]);
    params[nparams].powerq = atof(words[13]);
                       ...
                       double *fj, double *fk, int eflag, double &eng)
{
  ...
  double rinv12,cs,delcs,delcssq,facexp,facrad,frad1,frad2,beta3;
  ...
  facrad = paramijk->lambda_epsilon * facexp*delcssq + beta3 * facexp;
